function [grains, grainsOR, grainsSub, grainsRex,outdata] = ebsd_fraction_recrystallized_with_og(grains,varargin)
...
critECD = 3; % [um]
%critGOS = 1.5*degree; % [degrees]
critGOS = 2.7*degree; %BRUK DENNE OM DU GJØR ALLE PÅ NYTT
...
% Set Original grains to 2
grains(grains.phase==0 & grains.ECD > critECD_OG & grains.GOS > critGOS_OG).RX = 2;
...
grainsOR = grains(grains.phase==0 & grains.RX==2);
numGrainsSub = length(grainsSub);
numGrainsRex = length(grainsRex);
numGrainsOR = length(grainsOR);

outdata = [numGrainsSub,numGrainsRex,numGrainsOR];
...
if to_plot
    ...
    if numGrainsOR ~= 0
        p5 = plot(grainsOR, grainsOR.RX, 'facecolor', 'g');
        hold on
        plots = [plots p5(1)];
        lgText{end + 1} = 'OG';
        alpha(p5, 0.2)
